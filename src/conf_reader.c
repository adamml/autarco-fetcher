        ...
        while(fgets(line, 2048, fid)){
            if(autarco == 1){
                if(strncmp(line, "site_id = ", 10) == 0){
                    ...
                    if(subString){strncpy(c->AUTARCO_SITE_ID, subString, 64);}
                } else if (strncmp(line, "uname = ", 8) == 0)
                {
                    subString = strtok(line, "\"");
                    subString = strtok(NULL, "\"");
                    if(subString){strncpy(c->AUTARCO_UNAME, subString, 256);}
                } else if (strncmp(line, "pwd = ", 6) == 0){
                    subString = strtok(line, "\"");
                    subString = strtok(NULL, "\"");
                    if(subString){strncpy(c->AUTARCO_PWD, subString, 256);}
                } else if (strncmp(line, "outdir = ", 9) == 0)
                {
                    subString = strtok(line, "\"");
                    subString = strtok(NULL, "\"");
                    if(subString){strncpy(c->AUTARCO_OUT_DIR, subString, 512);}
                }
            }
